...
- **STYLE THE CAST — say what everyone is wearing, in every prompt.** Leaving
  ...
  **wardrobe register** for this run: interpret it through THIS story's world, era
  ...
  - **Never on a lead**: a buttoned collared work shirt, a fastened blazer, a
    ...
    **This list describes MODERN dress and applies only to a present-day story.** In a
    historical world, high collars, buttoned bodices and covered arms are correct
    period clothing, and forcing modern cuts onto them is the worse error. There,
    glamour is a ball gown: bare shoulders, a low back, a cinched waist, rich fabric
    and jewellery — never a garment the era did not have.
...
- End every prompt with this negatives tail: "photorealistic live-action, real
  ...
  painting, not glossy, not airbrushed, not plastic skin, no clothing from the wrong
  century, no modern garments in a historical story, no extra text, no watermark,
  no logo".
- **In a PRESENT-DAY story only, add to that tail**: "no high neckline, no
  turtleneck, no mock neck, no buttoned-up collar, no shapeless or covered-up
  clothing". These exist because the image model follows the *colour* of a described
  outfit while quietly substituting a safer cut — a prompt asking for a lace-trimmed
  camisole came back as a high mock-neck dress in the right shade, so stating the cut
  positively is not enough on its own.
  **Do NOT add them to a historical story.** High collars and buttoned bodices are
  correct period dress, and banning them forces modern clothes into the wrong century
  — a worse error than a covered neckline.
