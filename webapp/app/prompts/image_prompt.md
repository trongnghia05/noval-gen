...
- End every prompt with this negatives tail: "photorealistic live-action, real
  ...
  painting, not glossy, not airbrushed, not plastic skin, no high neckline, no
  turtleneck, no mock neck, no buttoned-up collar, no shapeless or covered-up
  clothing, no extra text, no watermark, no logo".
  The wardrobe negatives are there because the image model follows the *colour* of a
  described outfit while quietly substituting a safer cut — a prompt asking for a
  lace-trimmed camisole came back as a high mock-neck dress in the right shade.
  Stating the cut positively is not enough on its own; it has to be denied here too.
