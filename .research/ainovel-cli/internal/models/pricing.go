...
const (
	...
	// maxModelAgeDays đồng bộ với gen_models.go: mô hình vượt quá số ngày này được coi là lỗi thời và bị loại bỏ.
	maxModelAgeDays = 730
)

// providerMap chuẩn hóa tiền tố vendor của OpenRouter thành tên nhà cung cấp nội bộ.
// Các nhà cung cấp không có trong danh sách sẽ bị bỏ qua để tránh trả về các mục không dùng được.
...
// StartPricingRefresh khởi động goroutine nền để làm mới dữ liệu mô hình.
// Ưu tiên đọc cache trên đĩa (TTL 24h); nếu hết hạn hoặc không tồn tại thì tải dữ liệu mới và ghi xuống đĩa.
// Nếu cacheDir rỗng thì bỏ qua cache đĩa, nhưng vẫn thử tải qua mạng.
...
func convertModel(m openRouterModel) (ModelEntry, bool) {
	...
	modelID := parts[1]
	// Bỏ qua hậu tố biến thể (ví dụ: :thinking / :free)
...
// isStaleModel lọc các mô hình lỗi thời theo maxModelAgeDays.
// Giá trị 0 hoặc âm được coi là dữ liệu thiếu và bị loại bỏ như mô hình cũ.
...
// tokenToMillion chuyển đổi "giá USD mỗi token" mà OpenRouter trả về thành "giá USD mỗi 1M token".
