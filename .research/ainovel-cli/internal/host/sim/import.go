...
func RunImport(ctx context.Context, st *store.Store, path string) (<-chan Event, error) {
	...
	go func() {
		...
		emit(StageImport, "Đang nhập hồ sơ mô phỏng...", nil)
		result, err := ImportProfile(ctx, st, path)
		if err != nil {
			emit(StageError, "Nhập hồ sơ mô phỏng thất bại", err)
			return
		}
		emit(StageDone, fmt.Sprintf("Đã nhập hồ sơ mô phỏng: thêm mới %d nguồn, bỏ qua trùng lặp %d nguồn", result.ImportedSources, result.SkippedSources), nil)
	}()
	return events, nil
}
