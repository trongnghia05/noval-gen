...
// completedBook tạo một cuốn tiểu thuyết N chương đã hoàn kết (phase=complete, CompletedChapters=1..n).
...
func TestReopenBookReopensCompletedBook(t *testing.T) {
	...
	if len(p.PendingRewrites) != 2 || p.PendingRewrites[0] != 3 || p.PendingRewrites[1] != 1 {
		t.Errorf("PendingRewrites = %v, want [3 1] (đưa vào hàng đợi nguyên trạng)", p.PendingRewrites)
	}
...
func TestReopenBookRejectsNonCompleteBook(t *testing.T) {
	// Sách đang viết (chưa hoàn kết) không thể reopen
	s := store.NewStore(t.TempDir())
	if err := s.Init(); err != nil {
		t.Fatalf("Init: %v", err)
	}
...
func TestReopenBookRejectsUnwrittenChapters(t *testing.T) {
	...
	// Chương 5 không tồn tại → từ chối (thuộc loại viết tiếp/vượt biên, cần đi qua điều chỉnh độ dài)
